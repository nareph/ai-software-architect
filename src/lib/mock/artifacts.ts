...
export const MOCK_ARTIFACTS = {
  business_analysis: {
    ...
    businessRules: [
      ...
      "Un abonnement mensuel donne droit à 8 cours par mois.",
    ...
    assumptions: [
      "Un seul établissement sportif dans un premier temps.",
      "Les coachs sont des employés de l'établissement.",
      "La devise est l'Euro uniquement.",
    ],
    outOfScope: [
  ...
  architecture: {
    overview: "Architecture monolithe modulaire déployée sur Vercel avec Neon PostgreSQL. Choix justifié par la taille d'équipe (1-3 développeurs), le budget limité et la nécessité d'itérer rapidement.",
    style: "Monolithe modulaire",
    justification: "Pour une équipe de 1 à 3 développeurs avec un budget cloud limité, le monolithe modulaire offre le meilleur rapport simplicité/maintenabilité.",
    stack: [
      ...
      { layer: "Backend", technology: "Next.js API Routes", version: "16.x", justification: "Évite un backend séparé pour le MVP.", alternatives: ["Express.js séparé", "Fastify"] },
      { layer: "Base de données", technology: "Neon PostgreSQL", version: "16.x", justification: "Serverless, compatible Vercel, free tier généreux.", alternatives: ["PlanetScale", "Supabase"] },
      { layer: "ORM", technology: "Drizzle ORM", version: "0.38+", justification: "Typesafe, léger, compatible Edge Runtime.", alternatives: ["Prisma", "Kysely"] },
      { layer: "Authentification", technology: "NextAuth.js v5", version: "5.x", justification: "Intégration native Next.js, JWT extensible.", alternatives: ["Clerk", "Auth0"] },
      { layer: "Paiement", technology: "Stripe", version: "latest", justification: "Standard de facto, documentation excellente.", alternatives: ["Paddle", "Lemon Squeezy"] },
      { layer: "Notifications", technology: "Resend + Twilio", version: "latest", justification: "Resend pour l'email transactionnel, Twilio pour les SMS.", alternatives: ["SendGrid", "Vonage"] },
      { layer: "Cache", technology: "Upstash Redis", version: "latest", justification: "Serverless, compatible Vercel.", alternatives: ["Redis Cloud", "Memcached"] },
    ],
    modules: [
      { name: "Auth Module", responsibility: "Authentification, sessions, gestion des rôles.", technology: "NextAuth.js v5", dependencies: [], exposedApis: ["/api/auth/*"] },
      { name: "Booking Module", responsibility: "Gestion des réservations, vérification des disponibilités.", technology: "Next.js API Routes + Drizzle", dependencies: ["Auth Module", "Notification Module"], exposedApis: ["/api/bookings"] },
      { name: "Subscription Module", responsibility: "Gestion des abonnements, intégration Stripe.", technology: "Next.js API Routes + Stripe SDK", dependencies: ["Auth Module"], exposedApis: ["/api/subscriptions", "/api/webhooks/stripe"] },
      { name: "Schedule Module", responsibility: "Gestion des plannings et des créneaux.", technology: "Next.js API Routes + Drizzle", dependencies: ["Auth Module"], exposedApis: ["/api/schedules"] },
      { name: "Notification Module", responsibility: "Envoi d'emails et SMS transactionnels.", technology: "Resend SDK + Twilio SDK", dependencies: [], exposedApis: [] },
    ],
    patterns: [
      { name: "Repository Pattern", justification: "Abstraction de la couche données pour faciliter les tests." },
      { name: "Webhook Pattern", justification: "Indispensable pour les événements Stripe asynchrones." },
      { name: "Optimistic UI", justification: "Amélioration de la réactivité perçue lors des réservations." },
    ],
    risks: [
      { description: "Dépendance forte à Stripe — tout changement de provider implique une refonte.", severity: "medium", mitigation: "Encapsuler Stripe derrière une interface abstraite PaymentProvider." },
      { description: "Performance des requêtes de calendrier lors de pics d'utilisation.", severity: "low", mitigation: "Mise en cache Redis des créneaux avec TTL de 5 minutes." },
    ],
    ...
    securityNotes: "RBAC via NextAuth, toutes les routes API vérifient le rôle. Webhooks Stripe signés et vérifiés.",
  },

  database_schema: {
    engine: "PostgreSQL",
    justification: "Choix naturel pour un système de réservation avec des relations complexes. Les garanties ACID sont indispensables pour éviter les doubles réservations.",
    tables: [
      {
        name: "users",
        ...
        columns: [
          ...
          { name: "email", type: "VARCHAR(255)", nullable: false, unique: true, primaryKey: false, foreignKey: null, default: null, description: "Email unique." },
      ...
      },
      {
        ...
        columns: [
          { name: "id", type: "UUID", nullable: false, unique: true, primaryKey: true, foreignKey: null, default: "gen_random_uuid()", description: "Identifiant unique." },
          { name: "coach_id", type: "UUID", nullable: false, unique: false, primaryKey: false, foreignKey: { table: "users", column: "id", onDelete: "CASCADE" }, default: null, description: "Coach responsable." },
      ...
      },
      {
        ...
        indexes: [
          ...
          { columns: ["schedule_id"], unique: false, description: "Réservations d'un créneau." },
        ],
      },
      {
        name: "subscriptions",
        ...
        columns: [
          ...
          { name: "stripe_subscription_id", type: "VARCHAR(255)", nullable: true, unique: true, primaryKey: false, foreignKey: null, default: null, description: "ID Stripe." },
          ...
          { name: "current_period_end", type: "TIMESTAMPTZ", nullable: true, unique: false, primaryKey: false, foreignKey: null, default: null, description: "Fin de période." },
    ...
    notes: [
      "Index unique sur (member_id, schedule_id) pour éviter les doubles réservations.",
      "Soft delete sur users pour conserver l'historique.",
    ],
  },

  diagrams: {
    // ── C4 remplacé par flowchart LR — rendu fiable dans Mermaid 11.x ──
    c4_container: `flowchart LR
  subgraph Actors
    M([Membre])
    C([Coach])
    A([Admin])
  ...
  subgraph Platform["Plateforme de Réservation"]
    direction TB
    WEB["Web App\nNext.js 16"]
    API["API Routes\nNext.js"]
    DB[("PostgreSQL\nNeon")]
  ...
  A->>DB: CHECK disponibilité
  DB-->>A: Disponible
  A->>DB: INSERT booking
  A->>S: Crée PaymentIntent
  S-->>A: client_secret
  A-->>W: client_secret
  W-->>M: Page de paiement
  ...
  users ||--o{ schedules : "coach"
  users ||--o{ bookings : "member"
  schedules ||--o{ bookings : "schedule"
  ...
  backlog: {
    epics: [
      { id: "EP-01", name: "Authentification & Profils", description: "Inscription, connexion, gestion des rôles.", priority: "critical" },
      { id: "EP-02", name: "Calendrier & Créneaux", description: "Consultation et gestion des créneaux.", priority: "critical" },
    ...
    stories: [
      ...
      },
      {
        ...
        acceptanceCriteria: ["L'erreur affichée ne précise pas lequel des champs est incorrect", "La session dure 24h avec renouvellement automatique", "Redirection vers la page demandée après connexion"],
      ...
      },
      {
        ...
        acceptanceCriteria: ["La réservation est impossible si le créneau est complet", "Un membre ne peut pas réserver deux cours au même horaire", "Une confirmation par email est envoyée immédiatement"],
      ...
      },
      {
        ...
        asA: "membre connecté", iWant: "souscrire à un abonnement mensuel ou annuel", soThat: "j'accède à un nombre de cours défini",
        priority: "high", storyPoints: 13,
        acceptanceCriteria: ["Les deux plans sont clairement présentés avec leurs avantages", "Le paiement est traité via Stripe Checkout", "L'abonnement est actif immédiatement après le paiement"],
        technicalNotes: "Stripe Checkout Session + webhook checkout.session.completed.",
        dependencies: ["US-002"],
      },
      {
        ...
        acceptanceCriteria: ["Un coach ne peut créer des créneaux qu'au minimum 7 jours à l'avance", "La capacité maximum est configurable par créneau", "Un créneau avec des réservations ne peut pas être supprimé"],
      ...
      },
      {
        ...
        acceptanceCriteria: ["Le rappel est envoyé par email ET SMS 24h avant le cours", "Le rappel contient le titre, l'horaire et le nom du coach", "Le rappel n'est pas envoyé si la réservation a été annulée"],
